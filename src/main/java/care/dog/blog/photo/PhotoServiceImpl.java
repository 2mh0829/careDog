...
public class PhotoServiceImpl implements PhotoService {
	...
	public int insertPhoto(Photo dto, String pathname) {
		...
		try {
			if(dto.getUpload()!=null && ! dto.getUpload().isEmpty()) {
				...
				int maxNum=dao.selectOne("blogPhoto.maxNum", map);
				dto.setNum(maxNum+1);
	...
	public int dataCount(Map<String, Object> map) {
		...
		try {
			result=dao.selectOne("blogPhoto.dataCount", map);
		} catch (Exception e) {
		}
	...
	public List<Photo> listPhoto(Map<String, Object> map) {
		...
		try {
			list=dao.selectList("blogPhoto.listPhoto", map);
		} catch (Exception e) {
		}
	...
	public Photo readPhoto(Map<String, Object> map) {
		...
		try {
			dto=dao.selectOne("blogPhoto.readPhoto", map);
		} catch (Exception e) {
		}
	...
	public Photo preReadPhoto(Map<String, Object> map) {
		...
		try {
			dto=dao.selectOne("blogPhoto.preReadPhoto", map);
		} catch (Exception e) {
		}
	...
	public Photo nextReadPhoto(Map<String, Object> map) {
		...
		try {
			dto=dao.selectOne("blogPhoto.nextReadPhoto", map);
		} catch (Exception e) {
		}
